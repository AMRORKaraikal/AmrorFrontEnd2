...
const SpecimenDetails = () => {
	...
	const getDetails = async (e) => {
		...
		try {
			const response = await fetch('https://amrorbackend.onrender.com/get-report-details', {
				method: 'POST',
				headers: {
		...
		setTimeout(async () => {
			...
			try {
				const report_data = {
					...
					ast: table,
					note: notes,
					report_date: reportDate.toUTCString(),
					reporter_name: reporterName,
					reporter_designation: reporterDesignation,
				}
				console.log(patient_id, specimen_id, report_data)
				const response = await fetch('https://amrorbackend.onrender.com/new-report', {
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
					},
	...
	// 					setIsCameraActive(false) // Stop the camera once a QR code is detected
	// 				}
	// 			}
	// 			img.src = imageSrc
	// 		}
							...
							<div className="flex flex-wrap gap-5 items-center justify-around my-14">
								<div>
									...
									<textarea
										...
										className="mt-3 w-[80%] h-[15vh] sm:h-24 sm:w-[30vw] border border-black p-4 text-lg rounded"
										required={true}
									/>
								</div>
								<div>
							...
							<div className="flex items-center flex-wrap justify-center gap-5 mb-8 mt-8">
								...
								<div className="flex flex-col justify-start">
									...
									<select
										...
										<option value="Semi-Resistant">Semi-Resistant</option>
										<option value="Sensitive">Senstive</option>
										{/* Add other options */}
									</select>

									<button className="mt-4 text-white bg-green-600 px-4 py-2 rounded-md" onClick={handleAdd}>
										Add
									</button>
										...
										return (
											<div key={index} id="data_div" className="w-full">
												<div className="flex">
													...
													<div className="border-2 border-black text-center bg-yellow-200 text-black text-lg p-4 w-1/12">
														<button className='cursor-pointer ' onClick={(e)=>handleClear(e,data.SNO)}>❌</button>
													</div>
												</div>
